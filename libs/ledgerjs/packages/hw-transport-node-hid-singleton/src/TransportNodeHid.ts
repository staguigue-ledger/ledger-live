...
export default class TransportNodeHidSingleton extends TransportNodeHidNoEvents {
  ...
  /**
   * OK this is not called from no event
   *
  ...
  static async disconnect() {
    ...
    if (transportInstance) {
      ...
      trace({
        type: LOG_TYPE,
        message: `Re-emitting "disconnect"`,
      });
  ...
  static open(
    ...
    return Promise.resolve().then(() => {
      ...
      const devicesDuringOpen = getDevices();
      tracer.trace(`Devices detected during open: ${devicesDuringOpen.length}`, {
        devicesDuringOpen,
      });

      const device = devicesDuringOpen[0];
      ...
      const onDisconnect = () => {
        if (!transportInstance) {
          tracer.trace("It should not even be called because we stopped listening to disconnect");
          return;
        }
        // TODO: not here
  ...
  setAllowAutoDisconnect(allow: boolean): void {
    this.tracer.trace("🦀 Allowing auto-disconnect", { allow });
  ...
  async close(): Promise<void> {
    this.tracer.trace("🦄 Calling close on HID Transport -> brut force auto disconnect", {
